...
//    the sum of elements not included (14 = 4 + 4 + 6). Therefore, the subsequence [7,6,7] is the minimal satisfying
//    the conditions. Note the subsequence has to returned in non-decreasing order.
//    Example 3:
//
//    Input: nums = [6]
//    Output: [6]
...
//            1 <= nums[i] <= 100
    ...
    public List<Integer> minSubsequence(int[] nums) {
        ...
        for (int i = nums.length - 1; i >= 0 && subSequenceSum <= totalSum/2; i--) {
    ...
    public static void main(String[] args) {
        int[] nums = {4,3,10,9,8};
        int[] nums2 = {4,4,7,6,7};
        MiniSubsequence res = new MiniSubsequence();
        System.out.println(res.minSubsequence(nums));
        System.out.println(res.minSubsequence(nums2));
    }
}
