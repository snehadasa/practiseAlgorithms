...
//            1 <= heights[i] <= 100
    ...
    public int heightChecker(int[] heights) {
        ...
        int checkerHeight[] = new int[heights.length];

        for (int i = 0, j = 0; i < heights.length; i++) {
            checkerHeight[j++] = heights[i];
        }
        Arrays.sort(checkerHeight);

        for (int i = 0; i < heights.length && i < checkerHeight.length; i++) {
            if (checkerHeight[i] != heights[i]) {
                count++;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(res.heightChecker(heights));
        System.out.println(res.heightChecker(heights2));
        System.out.println(res.heightChecker(heights3));
    }
}
