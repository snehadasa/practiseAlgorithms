...
//            1 <= accounts[i][j] <= 100
    ...
    public int maximumWealth1(int[][] accounts) {
        int maxSum = 0;
        for (int i = 0; i < accounts.length; i++) {
            int[] accountValues = accounts[i];
            int sum = 0;
            for (int accountValue : accountValues) {
                sum += accountValue;
            }
            if (sum > maxSum) {
                maxSum = sum;
    ...
    public static void main(String[] args) {
        ...
        MaximumWealth res = new MaximumWealth();
        System.out.println(res.maximumWealth2(acc));
        System.out.println(res.maximumWealth2(acc1));
    }
}
