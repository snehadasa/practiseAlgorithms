...
//            1 <= prices[i] <= 10^3
    ...
    public int[] finalPrices(int[] prices) {
        int newA[] = new int[prices.length];

        for (int i = 0; i < prices.length; i++) {
            for (int j = i+1; j < prices.length; j++) {
                if (prices[j] <= prices[i]) {
                    newA[i] = prices[i] - prices[j];
                    break;
                } else
                    newA[i] = prices[i];
            }
            newA[prices.length - 1] = prices[prices.length - 1];
        }
        return newA;
    }
    ...
    public static void main(String[] args) {
        ...
        FinalPrices res = new FinalPrices();
        System.out.println(Arrays.toString(res.finalPrices(prices)));
        System.out.println(Arrays.toString(res.finalPrices(prices1)));
    }
}
