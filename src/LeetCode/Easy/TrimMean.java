...
//            0 <= arr[i] <= 105
    ...
    public double trimMean(int[] arr) {
        int newLen = 0, finalLen = 0;
        int percentage = (int)(arr.length * (5.00000/100.00000));
        newLen = arr.length - percentage;
        finalLen = newLen - percentage;
        Arrays.sort(arr);

        double sum = 0;
        for (int i = percentage; i < newLen; i++) {
            sum += arr[i];
        }
        double mean = sum / finalLen;
        return mean;
    }

    public static void main(String[] args) {
        TrimMean res = new TrimMean();
        ...
        System.out.println(res.trimMean(arr));
        System.out.println(res.trimMean(arr1));
        System.out.println(res.trimMean(arr2));
        System.out.println(res.trimMean(arr3));
        //System.out.println(Arrays.toString(res.trimMean(arr)));
    }
}
