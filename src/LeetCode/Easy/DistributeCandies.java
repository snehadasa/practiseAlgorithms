...
//    On the fourth turn, ans[0] += 4, and the final array is [5,2,3].
    ...
    public int[] distributeCandies(int candies, int num_people) {
        int []numOfCandies = new int[num_people];
        int currentCandies = 1;
        int currentIndex = 0;

        while (candies > 0) {
            if (candies < currentCandies) {
                numOfCandies[currentIndex] += candies;
                break;
            }
            numOfCandies[currentIndex++] += currentCandies;
            candies -= currentCandies;
            currentCandies++;

            if (currentIndex == numOfCandies.length) {
                currentCandies = 0;
            }
        }
        return numOfCandies;
    ...
    public static void main(String[] args) {
        ...
        DistributeCandies res = new DistributeCandies();
        System.out.println(Arrays.toString(res.distributeCandies(candies, num_people)));
    }
}
