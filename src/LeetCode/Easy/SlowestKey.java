...
//    arrays are 0-indexed. The 0th key was pressed at the time 0, and every subsequent key was pressed at the exact
...
//    keysPressed contains only lowercase English letters.
    ...
    public char slowestKey(int[] releaseTimes, String keysPressed) {
      List<Integer> prevDiff = new ArrayList<>();

      prevDiff.add(releaseTimes[0]);
      for (int i = 1; i < releaseTimes.length; i++) {
          prevDiff.add(releaseTimes[i] - releaseTimes[i-1]);
      }

      int max = prevDiff.get(0);
      char res = keysPressed.charAt(0);
        for (int i = 0; i < prevDiff.size(); i++) {
            if (prevDiff.get(i) > max) {
                max = prevDiff.get(i);
                res = keysPressed.charAt(i);
            } else if (prevDiff.get(i) == max) {
    ...
    public static void main(String[] args) {
        ...
        int[] releasetimes2 = {1, 2};
        String key1 = "spuda";
        String key2 = "ba";
        SlowestKey res = new SlowestKey();
        System.out.println(res.slowestKey(releasetimes1, key1));
        System.out.println(res.slowestKey(releasetimes2, key2));
    }
}
