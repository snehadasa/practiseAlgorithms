...
//    The answer output can be returned in any order.
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String domain:cpdomains) {
            boolean found = true;
            String[] spaceSplitArray = domain.split(" ");
            int count = Integer.parseInt(spaceSplitArray[0]);
            String[] dotSplitArray = spaceSplitArray[1].split("\\.");
            String myStr = "";
            for (int i = dotSplitArray.length - 1; i >= 0; i--) {
                if (found) {
                    myStr = dotSplitArray[i] + myStr;
                    found = false;
                } else {
                    myStr = dotSplitArray[i] + "." + myStr;
                }
                ...
                if (map.containsKey(myStr)) {
                    int newCount = map.get(myStr);
                    map.put(myStr, count+newCount);
                } else {
                    map.put(myStr, count);
                }
            }
        ...
        for (Map.Entry<String, Integer> entered:map.entrySet()) {
            String countAndStr = entered.getValue().toString() + " " + entered.getKey();
            result.add(countAndStr);
        }
    ...
    public static void main(String[] args) {
        String[] n = {"9001 discuss.leetcode.com"};

        SubDomainVisits res = new SubDomainVisits();
        System.out.println(res.subdomainVisits(n));
    }
}
