...
//    How to make sure the duplicated files you find are not false positive?
    ...
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for (String path: paths) {
            String[] splitStringPath = path.split(" ");
            for (int i = 1; i < splitStringPath.length; i++) {
                String[] splitEachStringPath = splitStringPath[i].split("\\(");
                splitEachStringPath[1] = splitEachStringPath[1].replace(")", "");
                List<String> list = map.getOrDefault(splitEachStringPath[1], new ArrayList<>());
                list.add(splitStringPath[0] + "/" + splitEachStringPath[0]);
                map.put(splitEachStringPath[1], list);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(res.findDuplicate(paths));
    }
}
